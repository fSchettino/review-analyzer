...
{
    public function __construct(HotelsServiceInterface $hotelsServiceInterface, RulesServiceInterface $rulesServiceInterface)
    {
    ...
    {
        // Get hotel info
        $hotelInfo = $this->hotelsServiceInterface->find($hotelId);
        ...
        foreach ($reviewPhrases as $phrase) {
            ...
            foreach ($hotelInfo->rules as $rule) {
                $ruleInfo = $this->rulesServiceInterface->find($rule->id);
